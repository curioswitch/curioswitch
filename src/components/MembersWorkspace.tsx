...
function material(color: number, roughness = 0.55) {
...
function addClothingPattern(body: THREE.Group, pattern: WorkspacePattern) {
  const addFrontBox = (
    width: number,
    height: number,
    color: number,
...
function createWorker(
  ...
  body.add(torso);
  addClothingPattern(body, pattern);
...
function createTable(width: number, color = palette.cream) {
...
function createChair(color: number) {
...
function createSofa(color: number) {
...
export function MembersWorkspace({
  ...
  useEffect(() => {
    ...
    const workers = plan.workers.map((workerPlan) => {
      ...
      if (workerPlan.seated) {
        const chair = createChair(palette.cream);
    ...
    if (plan.zones.includes("desks")) {
      for (const desk of [
        ...
        const engineerTable = createTable(3.45);
    ...
    if (plan.zones.includes("meeting")) {
      const meetingTable = createRoundTable();
      ...
      [
        ...
        const chair = createChair(palette.aqua);
    ...
    if (plan.zones.includes("making")) {
      const labTable = createTable(3.8);
      labTable.position.set(5.25, 0, 1.15);
      workspace.add(labTable);
      const designLaptop = createLaptop();
    ...
    if (plan.zones.includes("conversation")) {
      const conversationSofa = createSofa(palette.sky);
    ...
    if (plan.zones.includes("lounge")) {
      const loungeSofa = createSofa(palette.pink);
  ...
  return (
    <div className="relative h-64 w-full overflow-hidden bg-[#ffd83d] md:h-96">
      <canvas
        ref={canvasRef}
        role="img"
        aria-label={label}
        className="h-full w-full touch-pan-y cursor-pointer"
      />
      <p className="pointer-events-none absolute right-4 bottom-3 rounded-full bg-white/75 px-3 py-1 text-[10px] tracking-[0.16em] text-gray-700 uppercase backdrop-blur-sm md:right-6 md:bottom-5 md:text-xs">
        {hint}
      </p>
    </div>
  );
